...
void DecSyncBackend::loadItems(KCalendarCore::MemoryCalendar* cal, bool suppressSignals)
{
    if (!cal) {
        qWarning() << "DecSyncBackend::loadItems: Null calendar";
    ...
    auto existing = cal->incidences();
    for (const auto &inc : existing) {
    ...
    if (!coll && !todoColl) {
        qWarning() << "DecSyncBackend::loadItems: No collection for" << calId;
        if (!suppressSignals) emit calendarLoaded(cal);
        emit fetchFinished(calId, false, QStringLiteral("No collection found"));
        return;
    }
    ...
    const int total = resources.size() + todoResources.size();
    emit fetchStarted(calId, total);

    KCalendarCore::ICalFormat icalFormat;
    int current = 0;
    bool hybrid = isHybridCalendar(calId);

    auto loadFromResources = [&](const QMap<QString, DecSyncEntry> &res) {
        for (auto it = res.constBegin(); it != res.constEnd(); ++it) {
            QString icalData = it.value().value.toString();
            if (icalData.isEmpty()) {
                current++;
                emit fetchProgressChanged(calId, current, total);
                continue;
            }

            QList<KCalendarCore::Incidence::Ptr> incidences = deserializeIcal(icalData);
            for (const auto &inc : incidences) {
                // Only detect type violations for non-hybrid calendars
                if (!hybrid && !isTypeAllowed(calId, inc)) {
                    CalendarType expected = discoveredCalendarType(calId);
                    CalendarType actual = incidenceCalendarType(inc);
                    qWarning() << "DecSyncBackend::loadItems: Type violation -"
                                << inc->uid() << "is"
                                << (actual == CalendarType::Todo ? "VTODO" : "VEVENT")
                                << "in" << calId << "which expects"
                                << (expected == CalendarType::Todo ? "VTODO" : "VEVENT");
                    emit typeViolationDetected(calId, inc->uid(), expected, actual);
                }
    ...
    loadFromResources(resources);
    if (!todoResources.isEmpty()) {
        loadFromResources(todoResources);
    }
                                 ...
                                 const TranscodingPlan& plan)
{
    ...
    if (!hybrid) {
        DecSyncCollection *coll = collectionFor(calId);
        if (!coll) {
            qWarning() << "DecSyncBackend::storeItems: No collection for" << calId;
            return;
        }
    }
    ...
    for (const auto &item : finalItems) {
        ...
        if (!isTypeAllowed(calId, item)) {
            ...
            qWarning() << "DecSyncBackend::storeItems: Type violation -"
                        << item->uid() << "is"
                        << (actual == CalendarType::Todo ? "VTODO" : "VEVENT")
                        << "but collection" << calId << "expects"
                        << (expected == CalendarType::Todo ? "VTODO" : "VEVENT");
            emit calendarError(QString(), calId,
                QStringLiteral("Cannot store %1 in %2 collection: type mismatch")
                                 ...
                                 const TranscodingPlan& plan)
{
    ...
    if (!cal || item.isNull()) {
        qWarning() << "DecSyncBackend::updateItem: Invalid calendar or incidence";
        return;
    }
