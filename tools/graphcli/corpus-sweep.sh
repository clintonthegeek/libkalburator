...
# Creates CORPUS:-prefixed objects against the live Graph account via graphcli,
# captures the interesting endpoints into msgraph/captured/, and leaves cleanup
# to `graphcli sweep-clean` (run with --clean to invoke it at the end).
#
# Usage:
...
{"subject":"CORPUS: recurrence UNTIL","start":{"dateTime":"2026-09-03T14:00:00","timeZone":"UTC"},"end":{"dateTime":"2026-09-03T15:00:00","timeZone":"UTC"},"recurrence":{"pattern":{"type":"weekly","interval":1,"daysOfWeek":["thursday"],"firstDayOfWeek":"sunday"},"range":{"type":"endDate","startDate":"2026-09-03","endDate":"2026-12-31","recurrenceTimeZone":"UTC"}}}
EOF
) || return 1
    capture_all_views "rec-until" "$id"

    id=$(create_event rec-count <<'EOF'
{"subject":"CORPUS: recurrence COUNT","start":{"dateTime":"2026-09-03T16:00:00","timeZone":"UTC"},"end":{"dateTime":"2026-09-03T17:00:00","timeZone":"UTC"},"recurrence":{"pattern":{"type":"weekly","interval":2,"daysOfWeek":["thursday"]},"range":{"type":"numbered","startDate":"2026-09-03","numberOfOccurrences":6}}}
EOF
) || return 1
    capture_all_views "rec-count" "$id"

    id=$(create_event rec-noend <<'EOF'
{"subject":"CORPUS: recurrence noEnd","start":{"dateTime":"2026-09-04T09:00:00","timeZone":"UTC"},"end":{"dateTime":"2026-09-04T09:30:00","timeZone":"UTC"},"recurrence":{"pattern":{"type":"daily","interval":1},"range":{"type":"noEnd","startDate":"2026-09-04"}}}
EOF
) || return 1
...
{"subject":"CORPUS: absoluteMonthly day 13","start":{"dateTime":"2026-09-13T12:00:00","timeZone":"UTC"},"end":{"dateTime":"2026-09-13T13:00:00","timeZone":"UTC"},"recurrence":{"pattern":{"type":"absoluteMonthly","interval":1,"dayOfMonth":13},"range":{"type":"endDate","startDate":"2026-09-13","endDate":"2027-09-13"}}}
EOF
) || return 1
    capture_all_views "rec-abs-monthly" "$id"

    id=$(create_event rec-rel-monthly <<'EOF'
{"subject":"CORPUS: relativeMonthly second monday","start":{"dateTime":"2026-09-14T12:00:00","timeZone":"UTC"},"end":{"dateTime":"2026-09-14T13:00:00","timeZone":"UTC"},"recurrence":{"pattern":{"type":"relativeMonthly","interval":1,"daysOfWeek":["monday"],"index":"second","firstDayOfWeek":"sunday"},"range":{"type":"endDate","startDate":"2026-09-14","endDate":"2027-09-14"}}}
EOF
) || return 1
...
{"subject":"CORPUS: exception source series","start":{"dateTime":"2026-10-01T10:00:00","timeZone":"UTC"},"end":{"dateTime":"2026-10-01T11:00:00","timeZone":"UTC"},"recurrence":{"pattern":{"type":"weekly","interval":1,"daysOfWeek":["thursday"]},"range":{"type":"numbered","startDate":"2026-10-01","numberOfOccurrences":10}}}
EOF
) || return 1
...
for f in reversed(files):
    try:
        ...
        continue
    if any("CORPUS" in e.get("subject", "") for e in d.get("value", [])):
        print(f); break
PYEOF
)
...
vals=[e for e in d.get("value",[]) if e.get("subject","").startswith("CORPUS")]
...
{"subject":"CORPUS: OVERRIDDEN occurrence"}
EOF
...
{"subject":"CORPUS: all-day event","isAllDay":true,"start":{"dateTime":"2026-09-10T00:00:00","timeZone":"UTC"},"end":{"dateTime":"2026-09-11T00:00:00","timeZone":"UTC"}}
EOF
) || return 1
    capture_all_views "allday" "$id"

    id=$(create_event authored-et <<'EOF'
{"subject":"CORPUS: authored Eastern","start":{"dateTime":"2026-09-15T14:00:00","timeZone":"Eastern Standard Time"},"end":{"dateTime":"2026-09-15T15:00:00","timeZone":"Eastern Standard Time"},"isReminderOn":false}
EOF
) || return 1
...
{"subject":"CORPUS: two locations","start":{"dateTime":"2026-09-20T10:00:00","timeZone":"UTC"},"end":{"dateTime":"2026-09-20T11:00:00","timeZone":"UTC"},"location":{"displayName":"Primary place"},"locations":[{"displayName":"Primary place"},{"displayName":"Secondary place, remote"}]}
EOF
) || return 1
    capture_all_views "multilocation" "$id"

    id=$(create_event sensitivity <<'EOF'
{"subject":"CORPUS: private oof","start":{"dateTime":"2026-09-21T10:00:00","timeZone":"UTC"},"end":{"dateTime":"2026-09-21T11:00:00","timeZone":"UTC"},"sensitivity":"private","showAs":"oof","importance":"high","categories":["Purple category"],"reminderMinutesBeforeStart":45}
EOF
) || return 1
...
sweep_negative() {
    ...
    cat > "$file" <<'EOF'
{"subject":"CORPUS: NEG no timezone","start":{"dateTime":"2026-09-22T10:00:00"},"end":{"dateTime":"2026-09-22T11:00:00"}}
EOF
    ...
    cat > "$file" <<'EOF'
{"subject":"CORPUS: NEG month 13","start":{"dateTime":"2026-09-23T10:00:00","timeZone":"UTC"},"end":{"dateTime":"2026-09-23T11:00:00","timeZone":"UTC"},"recurrence":{"pattern":{"type":"absoluteYearly","interval":1,"dayOfMonth":1,"month":13},"range":{"type":"noEnd","startDate":"2026-09-23"}}}
EOF
...
sweep_contacts() {
    ...
    cat > "$file" <<'EOF'
{"givenName":"Ada","surname":"Lovelace","middleName":"Augusta","nickName":"Ada","fileAs":"Lovelace, Ada","jobTitle":"Analyst","department":"Engines","companyName":"Analytical Engines","businessPhones":["+1 613 555 0100"],"mobilePhone":"+1 613 555 0101","emailAddresses":[{"address":"ada@example.com","name":"Ada Lovelace"},{"address":"ada2@example.com","name":"Ada L"}],"homeAddress":{"street":"1 Charles St","city":"London","state":null,"countryOrRegion":"UK","postalCode":"W1J"},"birthday":"1815-12-10T00:00:00Z","categories":["CORPUS"]}
EOF
...
main() {
    ...
    if [[ "${2:-}" == "--clean" ]]; then
        ...
        echo "Left CORPUS objects in place for inspection. Clean up with: graphcli sweep-clean"
    fi
}
