...
2026-05-24 — Plan 3 Parts B & C — inv 4/5 post-review fixups (commits 89edbbb, 7f68e36) — initial Part B/C landing (single-agent run) left three loss contracts unbacked by code: VTODO Degraded-status, VTODO checklistItems/sortOrder Reversible, and iCal classification="personal" Degraded each claimed a verbatim stash that was never emitted; recurrence round-trip tests asserted substring not byte-identity. Fixed: originals now stashed as `X-CANON-*` props (round-trip into providerExtras), recurrence tests now assert byte-identical RRULE/EXDATE. Caught by retroactive spec-compliance review; logged as a process note — per-task review checkpoints were skipped when one subagent ran A5→Task13 unsupervised.
