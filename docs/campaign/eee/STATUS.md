...
2. Sanitize + commit Graph-side corpus fixtures (same two-pass sanitizer);
   promote one into a committed-fixture slot in `tst_ms_event_canon_edge`
   (mirroring the Google edge's `promoteCommittedLiveFixture`).
