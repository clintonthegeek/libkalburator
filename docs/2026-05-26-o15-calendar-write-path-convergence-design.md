...
The only genuine differentiator path (1) has is **transactional rollback**
(`SyncTransaction::rollbackCommitted`). This is out of scope to preserve: it is
mostly illusory for file-per-record / per-record-DAV backends, the converged
`DefaultBlobWriter` deliberately has no rollback, and if wanted it belongs at the
engine/write layer uniformly for all domains — not as a resurrected
calendar-specific transaction. Recorded as a non-goal in §6.
...
6. **Comment cleanup:** drop the stale `CalendarPluginWriter`/`SyncTransaction`
...
1. **Migrate write-failure injection.** `tst_calendar_sync_error_recovery` and
   `tst_engine_cancellation` inject write failures via `SyncTransaction` test
   hooks (`SYNC_HOOK`/`SyncTestHooks`). Re-point them at `MockBackend`'s existing
   failure-injection surface (`FailurePoint` / `setFailureMode`, as used by
   `tst_mockbackend_failure_injection`). Preserve each test's *assertion intent*
   (engine recovers / cancels cleanly on a mid-batch write failure); only the
   failure-injection vehicle changes. If a test slot turns out to assert purely
   on `SyncTransaction` internals (not engine behavior), delete that slot rather
   than contort it.
...
- **Lost transactional rollback** changes partial-failure semantics for
  multi-record calendar batches. Accepted (non-goal §2); converged behavior
  matches every other domain. Documented in FINDINGS when O15 is closed.
...
- **No new behavior** is introduced, so no new feature test is required. The one
  net-new test work is the **failure-injection migration** in Phase 2 (preserving
  the recovery/cancellation assertions on the new path).
