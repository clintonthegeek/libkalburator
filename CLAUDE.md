...
**Key findings this campaign:** O57–O66 (see FINDINGS.md). The ones that
shape daily work: O63 (grep edge-count pins in the same commit that grows
an edges() list; Graph type is dateTimeTimeZone but zone key is plain
timeZone), O64/O65 (crossing gates catch what per-component suites cannot),
O66+correction (carrier channels ALL survive via nav POSTs + filtered
collection-level expand; Outlook extension-id prefix is
Microsoft.OutlookServices.OpenTypeExtension.*; consumer contact GET-by-id
is flaky → drive contacts through listings/delta).

**Pending next actions (ordered):**
1. Sanitize task fixtures from machine-local captures → commit →
   promotion slots for both todo edges.
2. Annotate matrix + loss profiles with O66 carrier verdicts (regenerate).
3. A4 Phase-6 live checkpoint (USER-RUN per runbook).
