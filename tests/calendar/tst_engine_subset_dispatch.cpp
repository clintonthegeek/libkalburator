...
private:
    ...
    std::unique_ptr<CalendarBaselineStore> m_calBaselines;
...
void TestEngineSubsetDispatch::init()
{
    ...
    m_calBaselines    = std::make_unique<CalendarBaselineStore>(dbPath);
    ...
    m_engine->setCalendarBaselineStore(m_calBaselines.get());
