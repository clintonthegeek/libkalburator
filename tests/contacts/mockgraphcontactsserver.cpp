...
MockGraphContactsServer::MockGraphContactsServer(QObject *parent)
    : QObject(parent)
{
}

MockGraphContactsServer::~MockGraphContactsServer()
{
    stop();
}

bool MockGraphContactsServer::start()
{
    if (m_server)
        return true;
    ...
    connect(m_server, &QTcpServer::newConnection,
            this, &MockGraphContactsServer::onNewConnection);
    return true;
}

void MockGraphContactsServer::stop()
{
    if (!m_server)
        return;
    m_server->close();
...
quint16 MockGraphContactsServer::port() const
{
    return m_port;
}

QString MockGraphContactsServer::baseUrl() const
{
    return QStringLiteral("http://127.0.0.1:%1").arg(m_port);
}

void MockGraphContactsServer::setContactFolders(const QJsonArray &folders)
{
    m_folders = folders;
}

MockGraphContactsServer::Collection *
MockGraphContactsServer::findCollection(const QString &path)
{
...
bool MockGraphContactsServer::findContact(const QString &id,
...
void MockGraphContactsServer::addCollection(const QString &path,
...
void MockGraphContactsServer::setCollectionItems(const QString &path,
                                                 const QJsonArray &items)
{
...
void MockGraphContactsServer::addRoute(const QString &method, const QString &path,
                                       const QJsonValue &body, int status)
{
    m_routes.append(Route{ method, path, body, status });
}

QList<MockGraphContactsServer::RecordedRequest>
MockGraphContactsServer::requests() const
{
    return m_requests;
}

void MockGraphContactsServer::clearRequests()
{
    m_requests.clear();
}
...
void MockGraphContactsServer::onNewConnection()
{
...
void MockGraphContactsServer::handleSocket(QTcpSocket *socket)
{
    ...
    if (httpMethod == QStringLiteral("GET")
        && path == QStringLiteral("/v1.0/me/contactFolders")) {
        QJsonObject out;
        out.insert(QStringLiteral("@odata.context"),
                   baseUrl() + QStringLiteral("/$metadata#me/contactFolders"));
        out.insert(QStringLiteral("value"), m_folders);
        respond(socket, 200, QJsonDocument(out).toJson(QJsonDocument::Indented));
        return;
    }

    // ---- nav carrier protocol: POST .../contacts/{id}/extensions ----------
    if (httpMethod == QStringLiteral("POST")
        && path.startsWith(QStringLiteral("/v1.0/me/contacts/"))
        && path.endsWith(QStringLiteral("/extensions"))) {
        const QString contactId = path.mid(
            int(QStringLiteral("/v1.0/me/contacts/").size()),
            path.size() - int(QStringLiteral("/v1.0/me/contacts/").size())
                - int(QStringLiteral("/extensions").size()));
        Collection *c = nullptr;
        int idx = -1;
        if (!findContact(contactId, &c, &idx)) {
    ...
    // ---- resolve contacts collection / item paths -------------------------
    QString collectionPath;
    QString itemId;
    if (path == QStringLiteral("/v1.0/me/contacts")) {
        collectionPath = path;
    } else if (path.startsWith(QStringLiteral("/v1.0/me/contacts/"))) {
        collectionPath = QStringLiteral("/v1.0/me/contacts");
        itemId = path.mid(int(QStringLiteral("/v1.0/me/contacts/").size()));
    } else if (path.startsWith(QStringLiteral("/v1.0/me/contactFolders/"))) {
        const QString rest =
            path.mid(int(QStringLiteral("/v1.0/me/contactFolders/").size()));
        const int marker = rest.indexOf(QStringLiteral("/contacts"));
        if (marker > 0) {
            collectionPath = QStringLiteral("/v1.0/me/contactFolders/%1/contacts")
                                 .arg(rest.left(marker));
            itemId = rest.mid(marker + int(QStringLiteral("/contacts").size()));
    ...
    if (!collectionPath.isEmpty()) {
        ...
        if (!itemId.isEmpty()) {
            ...
            int idx = -1;
            const bool found = findContact(itemId, &owner, &idx);
        ...
        if (c && httpMethod == QStringLiteral("GET")) {
            ...
            out.insert(QStringLiteral("@odata.context"),
                       baseUrl()
                           + QStringLiteral("/$metadata#me/contacts"));
...
void MockGraphContactsServer::respondNotFound(QTcpSocket *socket)
{
    QJsonObject err;
...
void MockGraphContactsServer::respond(QTcpSocket *socket, int status,
                                      const QByteArray &body,
